...
從你的日誌可以看到，系統同時運行了兩個不同的「角色」：

1.  `__adk_agent_builder_assistant` **(Builder 助手)**： 這是 ADK Web 介面內建的「助手」。當你在開發者介面點擊、瀏覽、或是讓系統自動生成配置時，ADK 預設會使用最強大的 **Gemini 2.5 Pro** 來擔任這個「引導開發者」的角色。這就是為什麼日誌中一直出現 `model: gemini-2.5-pro`。
    
2.  **你的自定義 Agent (mcp)**： 在日誌的中後段（12:57:25），你可以看到有一行： `INFO - google_llm.py:181 - Sending out request, model: gemini-2.5-flash` 這代表當系統真正執行你寫的 `root_agent.yaml` 或 `research_agent.yaml` 時，它**確實有切換回你設定的 Flash 模型**。
